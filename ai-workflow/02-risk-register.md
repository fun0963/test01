...
| RK-004 | P3 | 非 UTF-8 / 損壞編碼檔會拋 `UnicodeDecodeError`（`ValueError` 子類，非 `OSError`），繞過 `error:` 路徑印 traceback，違反 G2 訊息格式 | 邊界輸入下錯誤輸出非預期格式（exit 仍為 1） | Orchestrator | PR #3 仲裁 Accepted（`reviews/T-001-review.md` R-001）；已開窄修復票 T-003；建議 merge 前先跑 | Open（待 T-003） |
