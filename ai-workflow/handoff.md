...
## Current Status
- T-001 實作完成（PR #3 已 merge 進 main），smoke 全綠；T-002 unittest 框架完成（PR #6 已 merge 進 main），Unit gate 綠燈；T-003（UnicodeDecodeError 修復）已開票；T-004（test_empty_file stderr 斷言補強）待排
...
- 優先執行 T-003（窄修復 UnicodeDecodeError），使 G2 格式完整；後接 T-004（test_empty_file stderr 斷言補強）；最後考慮額度記錄、白名單收緊等後續工項
